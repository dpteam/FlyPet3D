...
{
	private sealed class UnboundedWriter : ChannelWriter<T>
	{
		private readonly UnboundedChannel<T> _parent;

		public UnboundedWriter(UnboundedChannel<T> parent)
		{
			_parent = parent;
		}
		...
		public override Task WriteAsync(T item, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			if (!_parent.TryWriteInternal(item))
			{
	...
	private sealed class UnboundedReader : ChannelReader<T>
	{
		private readonly UnboundedChannel<T> _parent;

		public UnboundedReader(UnboundedChannel<T> parent)
		{
			_parent = parent;
		}
	...
	private readonly UnboundedChannelOptions _options;

	public override ChannelReader<T> Reader { get; }

	public override ChannelWriter<T> Writer { get; }

	public UnboundedChannel(UnboundedChannelOptions options)
	{
		_options = options;
		Reader = new UnboundedReader(this);
		Writer = new UnboundedWriter(this);
	}
